...
def app():
    ...
    st.markdown("### Upload scored csv file(s) for model performance report")
    st.write("\n")

    # Code to read a single file
    for period in st.session_state.time_period:

        uploaded_file = st.file_uploader("Choose scored " + period + " file - ", type = ['csv', 'xlsx'])
        global data
        data = {}
        if uploaded_file is not None:
            try:
                data[period] = pd.read_csv(uploaded_file)
            except Exception as e:
                print(e)
                data[period] = pd.read_excel(uploaded_file)
            st.dataframe(data[period].head(100))
